...
class HangmanGame:
    def __init__(self, master):
        ...
        "AMOUNT", "ARGUMENT", "ART", "BEAUTIFUL", "BELIEF", "CAUSE", "CERTAIN", "CHANCE", "CHANGE",
        "CLEAR", "COMMON", "COMPARISON", "CONDITION", "CONNECTION", "COPY", "DECISION"]

        self.secret_word, self.revealed_letter = self.choose_secret_word()
        self.correct_guesses = {self.revealed_letter}
    ...
    def initialize_gui(self):
        ...
        self.word_display = tk.Label(self.master, text="_ " * len(self.secret_word),
                                      font=("Helvetica", 30), bg='light blue')
        self.word_display.pack(pady=(40, 20))

        self.hint_label = tk.Label(self.master, text=f"Hint: The word contains the letter '{self.revealed_letter}'",
    ...
    def setup_alphabet_buttons(self):
        ...
        for letter in upper_row:
            button = tk.Button(upper_frame, text=letter, command=lambda l=letter:
            self.guess_letter(l), width=4, height=2, bg=button_bg, fg=button_fg, font=button_font)
            button.pack(side="left", padx=2, pady=2)

        for letter in lower_row:
            button = tk.Button(lower_frame, text=letter, command=lambda l=letter:
            self.guess_letter(l),
                                width=4, height=2, bg=button_bg, fg=button_fg,
                                font=button_font)
            button.pack(side="left", padx=2, pady=2)

    def choose_secret_word(self):
        word = random.choice(self.word_list)
        revealed_letter = random.choice(list(set(word)))
    ...
    def guess_letter(self, letter, button=None):
    ...
    def update_word_display(self):
        displayed_word = " ".join([letter if letter in self.correct_guesses else "_"
    ...
    def display_game_over_message(self, message):
        ...
        self.game_over_label = tk.Label(self.master, text=message, font=stylish_font,
                                          fg="red", bg='light blue')
        self.game_over_label.pack(pady=(10, 20))
        if not hasattr(self, 'restart_button'):
            self.restart_button = tk.Button(self.master, text="Restart Game",
                command=self.reset_game, width=20, height=2, bg=button_bg, fg=button_fg,
                font=button_font)
    ...
    def reset_game(self):
        ...
        self.hint_label.config(text=f"Hint: The word contains the letter '{self.revealed_letter}'")
